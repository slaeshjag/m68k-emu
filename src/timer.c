...
static void timer_trigger(int timer_no) {
	...
	if (timer.interrupt_enable & (1 << timer_no)) {
		interrupt_trig(CHIPSET_IO_PORT_NEW_TIMER);
	}
}
...
void timer_do_write_lword(uint32_t addr, uint32_t data) {
	...
	if ((addr & 0xFC) == 0x0) {
		...
		else if ((addr & 0xC) == 0x4)
			timer.timer[((addr & 0x30) >> 4) - 1].compare = data;
		else if ((addr & 0x8) == 0x8) {
			uint32_t oldstat = timer.timer[((addr & 0x30) >> 4) - 1].status;
			if ((!(oldstat & 1)) && (data & 0x1)) {
				timer.timer[((addr & 0x30) >> 4) - 1].count = 0;
			}

			timer.timer[((addr & 0x30) >> 4) - 1].status = data & 3;
		}
	}
}
